...
public class DrawApp extends JFrame {
    ...
    /*<exercise chapter="C" section="6" type="programmin" number="2">*/
    /** Load and instantiate the figure prototypes based on the names
     *  of the Drawable classes listed in the command line
     *  argument.
     *  @param args An array of strings containing Drawable class
     *  names.
     */
    private void loadFigKinds(String args[]) {
        // Load the figure kinds
        if (args.length == 0) {
            System.err.println("No figure kinds specified.");
            System.exit(1);
        }
        theFigKinds = new Drawable[args.length];
        for (int i = 0; i < args.length; i++) {
            try {
                theFigKinds[i] =
                        (Drawable) Class.forName(args[i]).newInstance();
            } catch (Exception ex) {
                System.err.println("Exception while instantiating "
                        + args[i]);
    ...
    /*<exercise chapter="C" section="6" type="programming" number="3">*/
