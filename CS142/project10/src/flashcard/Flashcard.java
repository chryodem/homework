...
public class Flashcard {
	Word theWords[]; //creates an array theWords
	public ArrayList<Word> theWordList;
	public Flashcard() {
		theWords = new Word[0];  //initializes the array with size of 0
	}
	...
	public String toString() {
...

		{
			...
			// words2D[i][0] = theWords[i].getVocabWord();
			// words2D[i][1] = theWords[i].getDefinition();
			
			//if successful
			//make new Panel(pass in the 2D array)
			//add myPanel object to JFrame
			

		}
		// System.out.println(storeAll);

		//FlashcardWindows flashcardWindows = new FlashcardWindows();
		// flashcardWindows.arrayOfWords(theWords[]);
	...
	public String[][] conversion() //creates 2d array
	{
...

		{
			...
			System.out.println(words2D[i][0]);
		...
		try {
			...
			if (file.exists()) {
				...
				try {
					...
					ArrayList<Word> theWordsList = new ArrayList<Word>();
					...
					for (int index = 0; index < nodeList.getLength(); index++) {
						...
						if (node.getNodeType() == Node.ELEMENT_NODE) {
							...
							NodeList nameNode1 = element
									.getElementsByTagName("def");
								...
								if (nameNode.item(iIndex).getNodeType() == Node.ELEMENT_NODE) {
									...
									//System.out.println("Defenition = "
										//	+ nameElement1.getFirstChild()
											//		.getNodeValue().trim()
											//+ "\n");
									String stringDefinition = nameElement1
											.getFirstChild().getNodeValue()
											.trim();

									Word w = new Word(stringWord,
											stringDefinition);
									theWordsList.add(w);
//									int temp = 0;
//									System.out.println(theWordsList.get(temp).toString());
//									temp++;
								}
							}
